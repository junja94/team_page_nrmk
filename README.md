...
- **PI profile**: `profile/profile.json`. Header fields (`name`, `photo`, `role`, `bio`, `links`, `scholarUrl`), then `experience`, `education`, `publications` (year, title, authors, venue, optional award and link), `awards`, `students`, `talks`. The `mosaic` block lists six tiles with a `caption` and an optional `src` (image or `.mp4`, plus `poster`); tiles without a source show a placeholder. `mosaic.layout` picks a preset: `grid` (the default here: equal tiles, `mosaic.columns` per row, `mosaic.aspect` such as `"4 / 3"`), or the irregular `hero`, `even`, `strip`, `feature` mosaics whose row height is `mosaic.rowHeight` in px. The CV is `media/cv.pdf`.
